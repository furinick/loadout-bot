...
function parseWeaponArray(arr) {
    ...
    for (const i of [1, 2, 3, 6]) {
        if (isClassname(arr[i])) {
            entries.push({ classname: arr[i], count: 1 });
        }
    }
 ...
 * ]
 */
function parseContainerItems(items) {
    const entries = [];
 ...
 */
export function parseLoadout(loadoutStr) {
    const entries = [];
    try {
