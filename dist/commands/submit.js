...
export async function execute(interaction) {
    ...
    if (loadoutChannel?.isTextBased() && !loadoutChannel.isDMBased()) {
        const embed = new EmbedBuilder()
            ...
            .addFields({ name: 'Squad', value: player.squad, inline: true }, { name: 'Role', value: player.role, inline: true }, { name: 'Weight', value: weight !== null ? `${weight} kg` : 'N/A', inline: true }, { name: 'Export Code', value: '```\n' + player.loadout.slice(0, 1000) + '\n```' })
